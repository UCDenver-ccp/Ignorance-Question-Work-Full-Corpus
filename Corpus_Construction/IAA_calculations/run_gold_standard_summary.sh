...
##copy all summary stuff to corpus construction
discontinuous_file='discontinuous_info_per_ontology.txt'
cp $all_file_path$preprocess_corpus$tokenized_files$discontinuous_file $all_file_path$corpus_construction

cp $all_file_path$preprocess_corpus$tokenized_files$discontinuous_file $all_file_path$corpus_path



eval_preprocess_summary_files='eval_preprocess_article_summary'
cp $all_file_path$eval_path$eval_preprocess_summary_files* $all_file_path$corpus_construction
cp $all_file_path$eval_path$eval_preprocess_summary_files* $all_file_path$corpus_path



##get BioC information from API
python3 collect_BioC_section_info.py -article_path=$all_file_path$corpus_path$articles -save_xml_path=$all_file_path$corpus_path$section_info_path


##run gold standard summary
python3 gold_standard_summary_stats.py -gs_path=$all_file_path$corpus_path -article_path=$articles -annotation_path=$concept_annotation -all_lcs_path=$all_lcs_path -section_info_path=$section_info_path -section_format=$section_format
