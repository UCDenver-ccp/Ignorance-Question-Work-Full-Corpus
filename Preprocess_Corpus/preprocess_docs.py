...
def gather_concept_annotations(ontology, concept_annotation_path, all_concept_annotation_file_paths_dict, filename, corpus, excluded_ignorance_types, all_lcs_dict, all_ignorance_types, extensions):
    ...
    if corpus.upper() == 'CRAFT':
        ...
        ##weird annotation errors:
        # old_ignorance_categories_dict = {'URGENT_CALL_TO_ACTION':'IMPORTANT_CONSIDERATION', 'ALTERNATIVE_OPTIONS':'ALTERNATIVE_OPTIONS_CONTROVERSY'}
        
        ##gold standard corpus from developmental set
        if 'SS' in corpus.upper():
            pass
        
        ##gold standard has the annotator names in it!
        ##Added annotators for all files - Gold standard v2
        # gold standard with all of us: emily, elizabeth, mayla, katie, and stephanie
        else: #TODO: changed!
            if all_concept_annotation_file_paths_dict.get(filename.split('.nxml')[0]):
                annotation_file_path = '%s%s' %(concept_annotation_path, all_concept_annotation_file_paths_dict[filename.split('.nxml')[0]])
            ...
            for annotation in root.iter('annotation'):
                ...
                for child in annotation:
                    ...
                    if child.tag == 'class':
                        ...
                        # ##fix the old ignorance types
                        # if ont_lc.upper() in old_ignorance_categories_dict.keys():
                        #     ont_lc = old_ignorance_categories_dict[ont_lc.upper()]
                        # else:
                        #     pass
                        # print('ont_lc', ont_lc)

                        ##TODO: binary ignorance OR MULTICLASS IGNORANCE
                        if ont_lc and ont_lc != 'subject_scope':

                            # ##binary ignorance
                            # it = 'ignorance'  # ignorance type category
                            if ont_lc.replace('0_', '').upper() in all_ignorance_types:
                                ...
                                if ont_lc.lower() in ['black_box_topic', 'components', 'environment', 'risk', 'sex', 'age', 'distinction_between_things', 'sr_partner', 'life_span', 'new_scope', 'mobilization', 'duration', 'establishes', 'time_points', 'gender', 'regulation', 'molecular_nature', 'options/specific_hypotheses', 'magnitude/amount/size', 'body_location', 'pathway', 'sr_partner_1', 'abilities', 'start_time', 'reproducibility', 'comparison', 'etiology']:
                                    empty_annotation = True

                                ##issues with annotations - 'THAN', 'IS'
                                # elif annotation_id in ['PMC2874300.nxml.gz-36', 'PMC2898025.nxml.gz-163','PMC2898025.nxml.gz-195', 'PMC2898025.nxml.gz-3', 'PMC2898025.nxml.gz-4', 'PMC2898025.nxml.gz-403', 'PMC2898025.nxml.gz-446', 'PMC2898025.nxml.gz-474']:
                                #     empty_annotation = True
                                else:
...
def preprocess_docs(craft_path, articles, concept_annotation, ontologies, output_path, pmcid_sentence_path, corpus, excluded_ignorance_types, all_lcs_path, extensions):
    ...
    if extensions:
        disc_output_path = output_path.replace(output_path.split('/')[-1],'') + 'discontinuous_info_per_ontology_EXT.txt'
    else:
        disc_output_path = output_path.replace(output_path.split('/')[-1], '') + 'discontinuous_info_per_ontology.txt'
    ...
    ##all lcs dictionary for ignorance
    if all_lcs_path:
        ...
        if corpus.upper() == 'IGNORANCE':
            all_lcs_dict['urgent_call_to_action'] = ['IMPORTANT_CONSIDERATION'] #switched name of urgent_call_to_action category to important consideration
            all_lcs_dict['than'] = ['ALTERNATIVE_OPTIONS_CONTROVERSY'] #from ontology of ignorance 2/11/20
            all_lcs_dict['alternative_options'] = ['ALTERNATIVE_OPTIONS_CONTROVERSY'] #combined alternative_options and controversy category together
        ...
        elif corpus.upper() == 'IGNORANCE_SS':
            all_lcs_dict['non_urgent_recommendation'] = ['FUTURE_WORK']
            all_lcs_dict['questions_answered_by_this_work'] = ['QUESTION_ANSWERED_BY_THIS_WORK']
            all_lcs_dict['problems_complications'] = ['PROBLEM_COMPLICATION']
            ...
            all_lcs_dict['epistemic_cues'] = ['IMPORTANT_CONSIDERATION']  #from one specific file where it is "critical" actually (PMC3933411.nxml.gz.xml)
            all_lcs_dict['some_evidence_exists'] = ['INCOMPLETE_EVIDENCE']
            all_lcs_dict['hard_to_study'] = ['DIFFICULT_TASK']


        # ensure that we have the correct ignorance type list in excluded
        if excluded_ignorance_types:
            for excluded_it in excluded_ignorance_types:
                if excluded_it not in set(all_ignorance_types):
                    print(set(all_ignorance_types))
                    raise Exception(
                        'ERROR ENTERING IN THE IGNORANCE TYPES TO EXCLUDE! (SEE ABOVE FOR THE LIST OF CORRECT TYPES)')


    ##add a list of all annotation files
    all_annotators = {'ELIZABETH', 'EMILY', 'KATIE', 'STEPHANIE', 'MAYLA', 'GOLD_STANDARD'}
    all_concept_annotation_file_paths_dict = {} #dict from pmc to filename
    for root, directories, filenames in os.walk(craft_path + concept_annotation):
        for filename in filenames:


            if filename.startswith('PMC') and filename.endswith('.xml'):
                # print(filename)
                # print(filename.split('.xml')[-2].replace('.', '').split('_'))
                current_annotators = filename.split('.xml')[-2].split('_')
                current_pmcid = filename.split('.nxml')[0]
                annotator_overlap = all_annotators and current_annotators
                if annotator_overlap:
                    if all_concept_annotation_file_paths_dict.get(current_pmcid):
                        raise Exception('ERROR: Issue with duplicate annotation files for the same PMCID')
                    else:
                        all_concept_annotation_file_paths_dict[current_pmcid] = filename
                else:
                    pass
                ...
                for filename in sorted(filenames):
                    ...
                    if filename.endswith('.txt'):# and '15040800' in filename: #loop over txt files to tokenize sentence and word levels #disjoint concept file: and '17696610' in filename
                        ...
                        with open(root+filename, 'r+', encoding='utf-8') as pmc_doc_file:
                            ...
                            ##dict: mention_ID -> (start_list, end_list, spanned_text, mention_class_ID, class_label, sentence_number)
                            # print('all_lcs dict', all_lcs_dict)
                            mention_ID_dict = gather_concept_annotations(ontology, craft_path+concept_annotation, all_concept_annotation_file_paths_dict, filename, corpus, excluded_ignorance_types, all_lcs_dict, all_ignorance_types, extensions)
