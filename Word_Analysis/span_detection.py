...
def convert_biotags_format(input_seq, bio_dict):
    output_seq = []
    for m in input_seq:
        s_tags = []
        for t in m:
            if t in bio_dict.keys():
                s_tags += [bio_dict[t]]
            else:
                s_tags += [t]
        output_seq += [s_tags]
...
def train_crf_per_ontology(tokenized_file_path, save_models_path, excluded_files, hyperparameter_dict, ontology,
    ...
    all_biotags_list = list(all_biotags_set)
    print(all_biotags_list)
    ...
    if hyperparameter_dict:
        ...
        y_pred_full2 = crf2.predict(X_train+X_test)
        y_pred_full_binary = []
        for y in y_pred_full2:
            if len(set(y)) == 1 and list(set(y))[0] == 'O':
        ...
        save_models_path, ontology, ontology, 'crf_model_full_5cv_pred_report_local'), 'w+') as report_file2:
            ...
            ##seqeval (BIO- tags)
            bio_dict = {'B': 'B-BEG', 'I':'I-IN', 'O-':'I-DISC'}

            y_train_seq = convert_biotags_format(y_train, bio_dict)
            y_test_seq = convert_biotags_format(y_test, bio_dict)
            y_pred_full2_seq = convert_biotags_format(y_pred_full2, bio_dict)
            y_pred2_seq = convert_biotags_format(y_pred2, bio_dict)


            report_file2.write('\n\n')
            report_file2.write('FULL REPORT FOR CLASSIFICATION (TRAINING FULL MODEL)\n')
            report_file2.write(classification_report(y_train_seq+y_test_seq, y_pred_full2_seq))  #y_train+y_test, y_pred_full2

            report_file2.write('\n\n')
            report_file2.write('TEST REPORT FOR CLASSIFICATION (CHEATING BECAUSE IN TRAINING DATA)\n')
            report_file2.write(classification_report(y_test_seq, y_pred2_seq))
        ...
        y_pred_full = crf.predict(X_train + X_test)
        ...
        save_models_path, ontology, ontology, 'crf_model_most_5cv_pred_report_local'), 'w+') as report_file:
            ...
            bio_dict = {'B': 'B-BEG', 'I': 'I-IN', 'O-': 'I-DISC'}
            y_train_seq = convert_biotags_format(y_train, bio_dict)
            y_pred_train_seq = convert_biotags_format(y_pred_train, bio_dict)
            y_test_seq = convert_biotags_format(y_test, bio_dict)
            y_pred_full_seq = convert_biotags_format(y_pred_full, bio_dict)
            y_pred_seq = convert_biotags_format(y_pred, bio_dict)


            report_file.write('\n\n')
            report_file.write('TRAINING REPORT FOR CLASSIFICATION (TRAINING DATA ONLY)\n')
            report_file.write(classification_report(y_train_seq, y_pred_train_seq))


            report_file.write('\n\n')
            report_file.write('FULL REPORT FOR CLASSIFICATION (ALL DATA)\n')
            report_file.write(classification_report(y_train_seq + y_test_seq, y_pred_full_seq))

            report_file.write('\n\n')
            report_file.write('TEST REPORT FOR CLASSIFICATION\n')
            report_file.write(classification_report(y_test_seq, y_pred_seq))
    ...
    # #hyperparameter optimization - regularization penalties
        ...
        with open('%s%s/%s_%s.txt' %(save_models_path, ontology, ontology, 'crf_full_model_hyperparameterization'), 'w+') as crf_hyperparams_file:
...
if __name__ == '__main__':
    ...
    if 'CRF' in algo:
        ...
        for ontology in ontologies:
            # print(args.crf_hyperparameters)
            if args.crf_hyperparameters:
                print('PROGRESS: RUNNING CRF WITH HYPERPARAMETERS AFTER TUNING')
