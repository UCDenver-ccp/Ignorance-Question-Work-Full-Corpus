...
python3 eval_preprocess_docs.py -craft_path=$all_file_path -concept_recognition_path=$all_file_path -eval_path=$all_file_path$eval_path -concept_system_output=$all_file_path -article_folder=$all_file_path$corpus_path$articles -tokenized_files=$tokenized_files -pmcid_sentence_files=$pmcid_sentence_files_path -concept_annotation=$concept_annotation -ontologies=$ontologies -evaluation_files=$training_files --gold_standard=$gold_standard
